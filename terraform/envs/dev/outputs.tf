...
output "auto_detected_workstation_public_ip" {
  value       = local.workstation_public_ip
  description = "Public workstation IP detected during Terraform plan/apply when auto detection is enabled."
}

output "effective_master_authorized_networks" {
  value       = local.effective_master_authorized_networks
  description = "Final GKE control-plane allowlist after merging manual CIDRs and the auto-detected workstation IP."
}
