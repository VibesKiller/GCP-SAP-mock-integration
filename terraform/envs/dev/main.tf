locals {
  ...
  workstation_public_ip = var.auto_detect_workstation_public_ip ? trimspace(data.http.workstation_public_ip[0].response_body) : null

  auto_detected_master_authorized_networks = var.auto_detect_workstation_public_ip ? [
    {
      cidr_block   = "${local.workstation_public_ip}/32"
      display_name = var.auto_detected_workstation_display_name
    }
  ] : []

  effective_master_authorized_networks = concat(
    var.master_authorized_networks,
    local.auto_detected_master_authorized_networks,
  )
...
data "http" "workstation_public_ip" {
  count = var.auto_detect_workstation_public_ip ? 1 : 0
  url   = var.workstation_public_ip_lookup_url
...
module "gke_cluster" {
  ...
  master_authorized_networks = local.effective_master_authorized_networks
